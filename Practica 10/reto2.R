...
for (iter in 1:tmax) {
  ...
  for(i in 1: length(obj)){
    if(p$fact[i]){
      ...
      proba <- c(proba, (.9*obj[i] + (1-obj[i])*.1)*.4)
    }
    
  }
  mantener <- sample(1:dim(p)[1], k, prob = proba)
  p <- p[mantener,]
  tam <- dim(p)[1]
  assert(tam == k)
...
stopCluster(cluster)
png("reto2.png", width=600, height=300)
plot(1:tmax, mejores, xlab="Paso", ylab="Mayor valor", type='l', ylim = c(min(mejores), optimo + 100))
